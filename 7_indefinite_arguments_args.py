...
def absolute_sum(*args):
    ...
    for num in args:
        total += abs(num)
#first time: total = 0 + abs(-1) = 1
#second time: total = 1 + abs(-2) = 3
#third time: total = 3 + abs(-3) = 6
...
def personal_numbers(name, *arg):
    sum_numbers = sum(arg)

    return(f"{name}, the sum of your numbers is {sum_numbers}")
print(personal_numbers("Alice", 5, 1, 2, 8))